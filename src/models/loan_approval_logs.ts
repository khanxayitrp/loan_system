...
export interface loan_approval_logsAttributes {
  ...
  action: 'submitted' | 'verified_basic' | 'verified_call' | 'verified_cib' | 'verified_field' | 'assessed_income' | 'verified_delivery_receipt' | 'approved' | 'rejected' | 'returned_for_edit' | 'cancelled';
  status_from?: string;
  status_to?: string;
  remarks?: string;
  performed_by: number;
...
export class loan_approval_logs extends Model<loan_approval_logsAttributes, loan_approval_logsCreationAttributes> implements loan_approval_logsAttributes {
  ...
  static initModel(sequelize: Sequelize.Sequelize): typeof loan_approval_logs {
    ...
    action: {
      type: DataTypes.ENUM('submitted','verified_basic','verified_call','verified_cib','verified_field','assessed_income','verified_delivery_receipt','approved','rejected','returned_for_edit', 'cancelled'),
      allowNull: false
    },
    status_from: {
