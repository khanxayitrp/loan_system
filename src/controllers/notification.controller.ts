...
export const fetchMySupperAppNotifications = async (req: Request, res: Response, next: NextFunction) => {
    try {
        // ดึงจาก Token (ป้องกันคนอื่นมาดูข้อมูลเรา)
        const userId = req.customerPayload?.userId; 
        const userType = req.customerPayload?.role === 'customer' ? RecipientType.CUSTOMER : RecipientType.STAFF;
...
export const markMySupperAppNotificationAsRead = async (req: Request, res: Response, next: NextFunction) => {
    try {
...
export const fetchMyMySupperAppUnreadCount = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const userId = req.customerPayload?.userId;
        const userType = req.customerPayload?.role === 'customer' ? RecipientType.CUSTOMER : RecipientType.STAFF;
