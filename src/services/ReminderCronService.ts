...
class ReminderCronService {
    ...
    public async processReminders() {
        ...
        try {
            // 🔐 1. ຍາດເອົາ Lock ຜ່ານ Redis
            await redisService.connect();
            const acquired = await redisService.setLock(LOCK_KEY, 'sending', LOCK_TIMEOUT);
            ...
            logger.warn(`[Reminder Batch] Redis lock failed - continue without lock: ${(error as Error).message}`);
        }
