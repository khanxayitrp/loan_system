...
class ProductGalleryService {
    ...
    async addImageGallery(productId: number, image_url: UploadedImage[], performedBy: number = 1): Promise<boolean> {
        ...
        try {
            ...
            // =========================================================================
            logger.info(`🧹 Starting MinIO file cleanup...`);
            ...
            if (filesToDeleteFromMinIO.length > 0) {
                logger.info(`🗑️ Found ${filesToDeleteFromMinIO.length} obsolete file(s) to delete from MinIO.`);
                ...
                fileUploadService.deleteFiles(filesToDeleteFromMinIO).catch(err => {
                    logger.error(`❌ Failed to delete files from MinIO:`, err);
                });
                
                logger.info(`✅ Successfully dispatched delete command to MinIO.`);
            } else {
                logger.info(`ℹ️ No obsolete files to delete from MinIO.`);
            }
