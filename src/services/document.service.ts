...
/**
  ...
  async uploadMultipleDocuments(
    ...
    const transaction = await db.sequelize.transaction();
    const uploadedPaths: string[] = []; // เก็บ path ที่เพิ่งอัปโหลดไป เผื่อต้อง Rollback
    ...
    try {
      ...
      for (const doc of documents) {
        ...
        const expiresAt = calculateExpiryDate(doc.doc_type);
      ...
      const createdDocs = await db.customer_documents.bulkCreate(newDocumentsData, { transaction });

      // 6. Commit Database ให้เสร็จสิ้น
      await transaction.commit();
      logger.info(`Successfully uploaded ${documents.length} documents for customer ${customer_id}`);
      ...
      return createdDocs.map(doc => doc.get({ plain: true })) as DocumentRecord[];

    } catch (error) {
      // ยกเลิก Database Changes
      await transaction.rollback();
      logger.error('Error uploading multiple documents, rolling back...', error);
