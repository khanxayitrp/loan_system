...
exports.allowViewDocument = exports.checkLoanOwnership = void 0;
...
const checkLoanOwnership = async (req, res, next) => {
    try {
        ...
        // 🟢 ตรวจสอบสถานะ
        const allowedStatuses = ['pending', 'verifying', 'verified', 'approved', 'disbursed', 'closed_early', 'completed'];
...
exports.checkLoanOwnership = checkLoanOwnership;
const allowViewDocument = async (req, res, next) => {
    try {
        // 1. เช็คชื่อ Parameter ให้ตรงกับ Route ของคุณ (ถ้า Route เป็น /:loanId ให้เปลี่ยนจาก application_id เป็น loanId)
        const paramId = req.params.application_id || req.params.loanId || req.body.application_id || req.body.loan_id;
        if (!paramId) {
        ...
        // 🟢 ตรวจสอบสถานะ
        const allowedStatuses = ['pending', 'verifying', 'verified'];
        if (!allowedStatuses.includes(loan.status) && loan.is_confirmed !== 0) {
            return res.status(400).json({ message: 'ໃບຄຳຂໍນີ້ຢູ່ໃນສະຖານະທີ່ບໍ່ສາມາດອັບໂຫຼດເອກະສານເພີ່ມໄດ້ແລ້ວ' });
        }
        next(); // ผ่านเงื่อนไขหมด ไปที่ Controller ต่อ
...
exports.allowViewDocument = allowViewDocument;
