...
exports.generateSolution1Key = generateSolution1Key;
exports.generateSolution2Key = generateSolution2Key;
exports.generateTransactionId = generateTransactionId;
exports.validateSMSParams = validateSMSParams;
exports.parseResultCode = parseResultCode;
exports.formatPhoneNumberForAPI = formatPhoneNumberForAPI;
 ...
 */
async function encryptKey(data, privateKey) {
    //   try {
    ...
    try {
        ...
        logger_1.logger.debug('Executing encrypt command:', command);
        const { stdout, stderr } = await execAsync(command);
        if (stderr) {
            logger_1.logger.error('Encrypt stderr:', stderr);
            throw new Error(`Encryption failed: ${stderr}`);
        }
        let encrypted = stdout.trim(); // JAR อาจ return ค่ามี newline
        logger_1.logger.debug('Encrypted result:', encrypted);
        // ตัด prefix "OK\r\n" หรือ "OK\n" ออก (ปัญหาจาก JAR return)
        if (encrypted.startsWith('OK')) {
            encrypted = encrypted.replace(/^OK\s*\r?\n/, '').trim();
        }
        logger_1.logger.debug('Cleaned encrypted result:', encrypted);
        return encrypted;
    }
    catch (error) {
        logger_1.logger.error('Failed to encrypt key with JAR', { error: error.message });
        throw new Error(`Encryption failed: ${error.message}`);
    }
}
/**
 ...
 */
async function decryptKey(encrypted, privateKey) {
    //   try {
    //     const keyBuffer = Buffer.from(privateKey.padEnd(24).slice(0, 24), 'utf-8');
    ...
    try {
        ...
        const { stdout, stderr } = await execAsync(command);
        if (stderr) {
            logger_1.logger.error('Decrypt stderr:', stderr);
            throw new Error(`Decryption failed: ${stderr}`);
        }
        return stdout.trim();
    }
    catch (error) {
        logger_1.logger.error('Failed to decrypt key with JAR', { error: error.message });
        throw new Error(`Decryption failed: ${error.message}`);
    }
}
/**
 ...
 */
async function generateSolution1Key(userId, transactionId, privateKey) {
    const data = `${userId.trim()}${transactionId.trim()}`;
    logger_1.logger.debug('Generating Solution 1 key', {
        userId,
        transactionId,
        dataToEncrypt: data,
    });
 ...
 * @returns Encrypted key
 */
async function generateSolution2Key(userId, transactionId, msisdn, privateKey) {
    const data = `${userId.trim()}${transactionId.trim()}${msisdn.trim()}`;
    logger_1.logger.debug('Generating Solution 2 key', {
        userId,
        transactionId,
        msisdn,
        dataToEncrypt: data,
 ...
 */
function generateTransactionId(prefix = 'OTP') {
...
function parseResultCode(resultCode) {
    ...
    logger_1.logger.debug('Parsed result code', {
        code: resultCode,
 ...
 */
async function testEncryption(userId, transactionId, msisdn, privateKey) {
    // Test Solution 1
