...
const fileUpload_service_1 = __importDefault(require("./fileUpload.service")); // Adjust path
...
class CheckListService {
    async CreateBasicVerification(data) {
        ...
        try {
            ...
            const checkLoanApp = await init_models_1.db.loan_applications.findByPk(loan_id, { transaction: t, lock: t.LOCK.UPDATE });
            if (!checkLoanApp) {
                throw new Error('Loan application not found');
            }
            const existingCustomer = await init_models_1.db.customers.findByPk(checkLoanApp.customer_id, { transaction: t, lock: t.LOCK.UPDATE });
            const existingBasicVerification = await init_models_1.db.loan_basic_verifications.findOne({
            ...
            if (existingBasicVerification) {
                ...
                // 🟢 Audit Log (UPDATE)
                await (0, auditLogger_1.logAudit)('loan_basic_verifications', existingBasicVerification.id, 'UPDATE', oldBasicData, basicVerificationData, performedBy, t);
                logRemark = 'ແກ້ໄຂຂໍ້ມູນການກວດສອບພື້ນຖານ (Updated Basic Verification)';
            }
            else {
                console.log('📝 Creating new basic verification...');
                basic_verification = await init_models_1.db.loan_basic_verifications.create(basicVerificationData, { transaction: t });
                // 🟢 Audit Log (CREATE)
                await (0, auditLogger_1.logAudit)('loan_basic_verifications', basic_verification.id, 'CREATE', null, basicVerificationData, performedBy, t);
            ...
            if (existingCustomer) {
                ...
                await (0, auditLogger_1.logAudit)('customers', existingCustomer.id, 'UPDATE', oldCustomerData, updateCustomerPayload, performedBy, t);
                const existingCustomerWorkInfo = await init_models_1.db.customer_work_info.findOne({
                ...
                if (existingCustomerWorkInfo) {
                    ...
                    await (0, auditLogger_1.logAudit)('customer_work_info', existingCustomerWorkInfo.id, 'UPDATE', oldWorkData, updateWorkPayload, performedBy, t);
                }
            }
            // Timeline Log
    ...
    async CreateCallVerification(data) {
        const t = await init_models_1.db.sequelize.transaction();
        try {
            const loan_id = data.loan_id || data.application_id;
            if (!loan_id) {
            ...
            await init_models_1.db.loan_applications.findByPk(loan_id, { transaction: t, lock: t.LOCK.UPDATE });
            let callsData = [];
            ...
            const incomingIds = [];
            const existingCount = await init_models_1.db.loan_call_verifications.count({
                where: { application_id: loan_id },
                transaction: t
            });
            for (const call of callsData) {
                ...
                if (call.id) {
                    const existingCall = await init_models_1.db.loan_call_verifications.findByPk(call.id, { transaction: t, lock: t.LOCK.UPDATE });
                    if (existingCall) {
                        ...
                        // 🟢 Audit Log (UPDATE)
                        await (0, auditLogger_1.logAudit)('loan_call_verifications', existingCall.id, 'UPDATE', oldCallData, payload, performedBy, t);
                    }
                }
                else {
                    const newCall = await init_models_1.db.loan_call_verifications.create(payload, { transaction: t });
                    incomingIds.push(newCall.id);
                    processedCalls.push(newCall);
                    // 🟢 Audit Log (CREATE)
                    await (0, auditLogger_1.logAudit)('loan_call_verifications', newCall.id, 'CREATE', null, payload, performedBy, t);
                }
            }
            // ลบข้อมูลเก่าที่ไม่ได้ส่งมา + 🟢 Audit Log (DELETE)
            let deleteCondition = { application_id: loan_id };
            if (incomingIds.length > 0) {
                deleteCondition.id = { [sequelize_1.Op.notIn]: incomingIds };
            }
            const callsToDelete = await init_models_1.db.loan_call_verifications.findAll({ where: deleteCondition, transaction: t });
            for (const item of callsToDelete) {
                await (0, auditLogger_1.logAudit)('loan_call_verifications', item.id, 'DELETE', item.toJSON(), null, performedBy, t);
            }
            await init_models_1.db.loan_call_verifications.destroy({ where: deleteCondition, transaction: t });
    ...
    async CreateCIBVerification(data) {
        const t = await init_models_1.db.sequelize.transaction();
        try {
            const loan_id = data.loan_id || data.application_id;
            if (!loan_id) {
            ...
            for (const detail of cibDetails) {
                ...
                const currentSeverity = CIB_SEVERITY[status] || 1;
                ...
                if (detail.id) {
                    const existingRecord = await init_models_1.db.loan_cib_history_details.findByPk(detail.id, { transaction: t, lock: t.LOCK.UPDATE });
                    if (existingRecord) {
                        ...
                        // 🟢 Audit Log (UPDATE)
                        await (0, auditLogger_1.logAudit)('loan_cib_history_details', existingRecord.id, 'UPDATE', oldDetailData, payloadDetail, performedBy, t);
                    }
                }
                else {
                    const newRecord = await init_models_1.db.loan_cib_history_details.create(payloadDetail, { transaction: t });
                    incomingIds.push(newRecord.id);
                    processedDetails.push(newRecord);
                    // 🟢 Audit Log (CREATE)
                    await (0, auditLogger_1.logAudit)('loan_cib_history_details', newRecord.id, 'CREATE', null, payloadDetail, performedBy, t);
                }
            }
            // ลบข้อมูลเก่าที่ไม่ได้ส่งมา + 🟢 Audit Log (DELETE)
            let deleteCondition = { application_id: loan_id };
            ...
            const detailsToDelete = await init_models_1.db.loan_cib_history_details.findAll({ where: deleteCondition, transaction: t });
            for (const item of detailsToDelete) {
                await (0, auditLogger_1.logAudit)('loan_cib_history_details', item.id, 'DELETE', item.toJSON(), null, performedBy, t);
            }
            await init_models_1.db.loan_cib_history_details.destroy({ where: deleteCondition, transaction: t });
            // ตารางหลัก (loan_cib_checks)
            const cibMainPayload = {
                application_id: loan_id,
            ...
            let cibMain = null;
            const existingMain = await init_models_1.db.loan_cib_checks.findOne({
                where: { application_id: loan_id },
                transaction: t,
            ...
            if (existingMain) {
                ...
                // 🟢 Audit Log (UPDATE)
                await (0, auditLogger_1.logAudit)('loan_cib_checks', existingMain.id, 'UPDATE', oldMainData, cibMainPayload, performedBy, t);
            }
            else {
                console.log('📝 Creating new CIB Main with status:', worstStatus);
                cibMain = await init_models_1.db.loan_cib_checks.create(cibMainPayload, { transaction: t });
                // 🟢 Audit Log (CREATE)
                await (0, auditLogger_1.logAudit)('loan_cib_checks', cibMain.id, 'CREATE', null, cibMainPayload, performedBy, t);
            }
    ...
    async findLocationUploadsDirectory() {
        const possiblePaths = [
            path_1.default.resolve(process.cwd(), 'public', 'uploads', 'locations'),
            path_1.default.resolve(process.cwd(), 'uploads', 'locations'),
            path_1.default.resolve(process.cwd(), 'src', 'public', 'uploads', 'locations'),
            path_1.default.resolve(process.cwd(), '..', 'public', 'uploads', 'locations'),
        ];
    ...
    async cleanupOrphanedLocationFiles(loanId) {
        // (ส่วนนี้ไม่เกี่ยวกับ DB Insert/Update ข้าม logAudit ได้ครับ)
        try {
            const dbRecords = await init_models_1.db.loan_field_visits.findAll({
            ...
            dbRecords.forEach((r) => {
                if (r.photo_url_1)
                    dbFileUrls.add(r.photo_url_1.trim());
                if (r.photo_url_2)
                    dbFileUrls.add(r.photo_url_2.trim());
            });
            const loan = await init_models_1.db.loan_applications.findByPk(loanId, { attributes: ['customer_id'] });
            if (!loan)
                return 0;
    ...
    async CreateFieldVisits(data) {
        ...
        try {
            ...
            for (const field of field_visit) {
                ...
                if (field.id) {
                    existingRecord = await init_models_1.db.loan_field_visits.findByPk(field.id, { transaction: t, lock: t.LOCK.UPDATE });
                }
                else {
                    existingRecord = await init_models_1.db.loan_field_visits.findOne({
                ...
                if (existingRecord) {
                    ...
                    // 🟢 Audit Log (UPDATE)
                    await (0, auditLogger_1.logAudit)('loan_field_visits', existingRecord.id, 'UPDATE', oldFieldData, payload, performedBy, t);
                }
                else {
                    const newFieldVisit = await init_models_1.db.loan_field_visits.create(payload, { transaction: t });
                    incomingIds.push(newFieldVisit.id);
                    processedFieldVisits.push(newFieldVisit);
                    // 🟢 Audit Log (CREATE)
                    await (0, auditLogger_1.logAudit)('loan_field_visits', newFieldVisit.id, 'CREATE', null, payload, performedBy, t);
                }
            }
            ...
            const visitsToDelete = await init_models_1.db.loan_field_visits.findAll({ where: deleteCondition, transaction: t });
            for (const item of visitsToDelete) {
                await (0, auditLogger_1.logAudit)('loan_field_visits', item.id, 'DELETE', item.toJSON(), null, performedBy, t);
            }
            await init_models_1.db.loan_field_visits.destroy({ where: deleteCondition, transaction: t });
            const existingCount = await init_models_1.db.loan_field_visits.count({ where: { application_id: loan_id }, transaction: t });
            const logRemark = existingCount > 0
            ...
            await t.commit();
            logger_1.logger.info(`✅ Field visits DB saved successfully! Total records: ${processedFieldVisits.length}`);
            this.cleanupOrphanedLocationFiles(loan_id).catch(e => {
                logger_1.logger.error('Failed to cleanup location images in background', e);
            });
            return { success: true, message: 'Field visits saved successfully', data: processedFieldVisits };
        }
        catch (error) {
            await t.rollback();
            logger_1.logger.error('❌ Error saving Field Visit:', error);
            return { success: false, message: error.message || 'Error saving Field Visit', data: null };
        }
    }
    async CreateIncomeAssessment(data) {
        const t = await init_models_1.db.sequelize.transaction();
        try {
            const loan_id = data.loan_id || data.application_id;
            if (!loan_id) {
            ...
            const existingIncomeAssessment = await init_models_1.db.loan_income_assessments.findOne({
                where: { application_id: loan_id },
                transaction: t,
                lock: t.LOCK.UPDATE
            });
            const incomeAssessmentData = {
                application_id: loan_id,
            ...
            if (existingIncomeAssessment) {
                ...
                // 🟢 Audit Log (UPDATE)
                await (0, auditLogger_1.logAudit)('loan_income_assessments', existingIncomeAssessment.id, 'UPDATE', oldIncomeData, incomeAssessmentData, performedBy, t);
            }
            else {
                console.log('📝 Creating new income assessment...');
                income_assessment = await init_models_1.db.loan_income_assessments.create(incomeAssessmentData, { transaction: t });
                // 🟢 Audit Log (CREATE)
                await (0, auditLogger_1.logAudit)('loan_income_assessments', income_assessment.id, 'CREATE', null, incomeAssessmentData, performedBy, t);
            }
    ...
    // ==========================================
    async GetBasicVerificationByLoanId(loan_id) {
        try {
            const basic_verification = await init_models_1.db.loan_basic_verifications.findOne({ where: { application_id: loan_id }, raw: true });
    ...
    async GetCallVerificationsByLoanId(loan_id) {
        try {
            const call_verifications = await init_models_1.db.loan_call_verifications.findAll({ where: { application_id: loan_id }, raw: true });
    ...
    async GetCIBCheckByLoanId(loan_id) {
        try {
            const mainData = await init_models_1.db.loan_cib_checks.findOne({ where: { application_id: loan_id }, raw: true });
            const historyDetails = await init_models_1.db.loan_cib_history_details.findAll({ where: { application_id: loan_id }, raw: true });
    ...
    async GetFieldVisitsByLoanId(loan_id) {
        try {
            const field_visits = await init_models_1.db.loan_field_visits.findAll({ where: { application_id: loan_id }, raw: true });
    ...
    async GetIncomeAssessmentByLoanId(loan_id) {
        try {
            const income_assessment = await init_models_1.db.loan_income_assessments.findOne({ where: { application_id: loan_id }, raw: true });
    ...
    async GetAllChecklistByLoanId(loan_id) {
        try {
            const basic_verification = await init_models_1.db.loan_basic_verifications.findOne({ where: { application_id: loan_id }, raw: true }) || null;
            const income_assessment = await init_models_1.db.loan_income_assessments.findOne({ where: { application_id: loan_id }, raw: true }) || null;
            const call_verifications = await init_models_1.db.loan_call_verifications.findAll({ where: { application_id: loan_id }, raw: true }) || [];
            const mainData = await init_models_1.db.loan_cib_checks.findOne({ where: { application_id: loan_id }, raw: true });
            const historyDetails = await init_models_1.db.loan_cib_history_details.findAll({ where: { application_id: loan_id }, raw: true });
...
exports.default = new CheckListService();
