...
exports.getLoanInstallmentDetails = exports.getCustomerLoanSummary = void 0;
const init_models_1 = require("../models/init-models"); // ปรับ path ตามโปรเจกต์ของคุณ
const sequelize_1 = require("sequelize");
const getCustomerLoanSummary = async (customerId) => {
    // 1. ดึงข้อมูลสินเชื่อทั้งหมดของลูกค้าที่กำลังผ่อน และผ่อนจบแล้ว
    const loans = await init_models_1.db.loan_applications.findAll({
        where: {
            customer_id: customerId,
            status: {
                [sequelize_1.Op.in]: ['disbursed', 'completed', 'closed_early']
            }
        },
        include: [
            {
            ...
            {
                model: init_models_1.db.repayments,
    ...
    for (const loan of loans) {
        ...
        repayments.sort((a, b) => a.installment_no - b.installment_no);
...
exports.getCustomerLoanSummary = getCustomerLoanSummary;
const getLoanInstallmentDetails = async (loanId) => {
    // 1. ดึงข้อมูลสินเชื่อ พร้อมตารางค่างวด
    const loan = await init_models_1.db.loan_applications.findOne({
        where: { id: loanId },
        include: [
            {
                model: init_models_1.db.products,
    ...
    if (!loan)
        throw new Error('ບໍ່ພົບຂໍ້ມູນສິນເຊື່ອ (Loan not found)');
    const loanData = loan.toJSON();
    const repayments = loanData.repayments || [];
    ...
    const installmentsList = repayments.map((rep) => {
...
exports.getLoanInstallmentDetails = getLoanInstallmentDetails;
