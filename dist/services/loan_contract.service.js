...
class LoanContractService {
    ...
    async createLoanContract(data) {
        ...
        try {
            ...
            await t.rollback();
            logger_1.logger.error('Create Loan Contract Error:', error.message);
            logger_1.logger.error(`Error stack: ${error.stack}`);
            logger_1.logger.error(`Error data: ${JSON.stringify(data)}`);
            throw error;
        }
    }
    async getLoanContract(loan_id) {
        try {
            const loanContract = await init_models_1.db.loan_contract.findOne({
...
exports.default = new LoanContractService();
