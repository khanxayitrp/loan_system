...
class PermissionService {
     ...
     */
    async getCombinedPermissions(userIds) {
        if (userIds.length === 0) {
            throw new Error('At least one user ID is required');
        }
        const permissions = await init_models_1.db.user_permissions.findAll({
        ...
        const featureCounts = {};
        const details = {}; // featureId -> [userIds ที่มี]
     ...
     */
    async updateBulkPermissions(payload, performedBy = 1) {
        ...
        if (allFeatureIds.length > 0) {
            const existingFeatures = await init_models_1.db.features.count({
        ...
        try {
            for (const { userId, featureIds } of payload) {
                ...
                const oldPermissions = await init_models_1.db.user_permissions.findAll({ where: { user_id: userId }, transaction });
                const oldFeatureIds = oldPermissions.map(p => p.feature_id);
                // Step 3: Delete old permissions
                await init_models_1.db.user_permissions.destroy({
                ...
                if (featureIds.length > 0) {
                    ...
                    await init_models_1.db.user_permissions.bulkCreate(data, { transaction });
                }
                // 🟢 บันทึก Audit Log (ใช้ userId เป็น recordId เพราะเป็นการแก้ไขสิทธิ์ของ User คนนั้นๆ)
                await (0, auditLogger_1.logAudit)('user_permissions', userId, 'UPDATE', { features: oldFeatureIds }, { features: featureIds }, performedBy, transaction);
                logger_1.logger.info(`Permissions updated for user ${userId} by admin: features=${featureIds.join(',')}`);
            }
            ...
            logger_1.logger.error('Bulk Update Error:', error);
            throw error;
        }
    }
    /**
    ...
    async getAllFeatures() {
        return await init_models_1.db.features.findAll({
            order: [['feature_name', 'ASC']]
        });
    }
    // Optional helper: สมมติฟังก์ชันเช็ค admin manage user ได้
    async canAdminManageUser(userId) {
     ...
     * 🟢 เพิ่ม performedBy เพื่อเก็บ Audit Log
     */
    async assignUserPermissions(userId, featureIds, performedBy = 1) {
        const transaction = await init_models_1.db.sequelize.transaction();
        try {
            ...
            // 🟢 บันทึก Audit Log 
            await (0, auditLogger_1.logAudit)('user_permissions', userId, 'UPDATE', { features: oldFeatureIds }, { features: featureIds }, performedBy, transaction);
            logger_1.logger.info(`Permissions assigned for user ${userId}: features=${featureIds.join(',')}`);
            await transaction.commit();
            return true;
        }
        catch (error) {
            await transaction.rollback();
            logger_1.logger.error('Assign User Permissions Error:', error);
            throw error;
        }
    }
    /**
     ...
     * 🟢 เพิ่ม performedBy เพื่อเก็บ Audit Log
     */
    async deleteAllUserPermissions(userId, performedBy = 1) {
        const transaction = await init_models_1.db.sequelize.transaction();
        try {
            // 🟢 ดึงข้อมูลสิทธิ์เดิมเก็บไว้ทำ Audit Log
            const oldPermissions = await init_models_1.db.user_permissions.findAll({ where: { user_id: userId }, transaction });
            ...
            await (0, auditLogger_1.logAudit)('user_permissions', userId, 'DELETE', { features: oldFeatureIds }, { features: [] }, performedBy, transaction);
            logger_1.logger.info(`All permissions deleted for user ${userId}`);
            await transaction.commit();
            return true;
        }
        catch (error) {
            await transaction.rollback();
            logger_1.logger.error('Delete All User Permissions Error:', error);
            throw error;
        }
    }
}
