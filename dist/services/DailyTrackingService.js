...
class DailyTrackingService {
    ...
    async processDailyPenalties() {
        ...
        try {
            ...
            hasLock = await redis_service_1.default.setLock(LOCK_KEY, 'processing', LOCK_TIMEOUT);
        }
        catch (error) {
            logger_1.logger.warn(`[Daily Batch] ⚠️ Redis unavailable, proceeding without lock:`, error);
        ...
        if (!hasLock) {
            logger_1.logger.info(`[Daily Batch] 🚫 Skip: Another server is already processing penalties.`);
            return;
        }
        const today = new Date();
        today.setHours(0, 0, 0, 0); // ตั้งค่าเวลาเป็นเที่ยงคืนเป๊ะของวันนี้
        logger_1.logger.info(`[Daily Batch] Start scanning for overdue schedules...`);
        ...
        while (hasMore) {
            ...
            const overdueSchedules = await init_models_1.db.repayments.findAll({
                where: {
                    due_date: { [sequelize_1.Op.lt]: today },
                    payment_status: { [sequelize_1.Op.ne]: 'paid' }
                },
                include: [
                    {
            ...
            for (const schedule of overdueSchedules) {
                ...
                try {
                    ...
                    const lockedSchedule = await init_models_1.db.repayments.findByPk(schedule.id, {
                        transaction,
                    ...
                    if (overdueDays > 0) {
                        ...
                        const appData = schedule.application;
                        // ดักจับ Error กันเหนียว เผื่อตารางข้อมูลขาดหาย
                        if (!appData) {
                            logger_1.logger.error(`[Daily Batch] Missing application data for Schedule ID ${schedule.id}`);
                            await transaction.rollback();
                            continue;
                        }
                        ...
                        await (0, auditLogger_1.logAudit)('repayments', lockedSchedule.application_id, 'UPDATE', oldData, updateSchedule.toJSON(), this.SYSTEM_USER_ID, transaction);
                    ...
                    logger_1.logger.error(`[Daily Batch] Failed to process schedule ID ${schedule.id}: ${error.message}`);
                }
            }
            // ขยับ Offset ไป Batch ถัดไป
            offset += BATCH_SIZE;
        }
        logger_1.logger.info(`[Daily Batch] Completed. Successfully updated ${totalUpdated} schedules.`);
    }
    startCronJob() {
        ...
        node_cron_1.default.schedule('1 0 * * *', async () => {
            ...
            logger_1.logger.info('⏰ Cron Job Triggered: Daily Penalty Calculation');
...
exports.default = new DailyTrackingService();
