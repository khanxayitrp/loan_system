...
async function testSendOTP() {
    ...
    try {
        ...
        logger_1.logger.error('Test send OTP failed', { error: error.message, stack: error.stack });
    }
}
async function testVerifyOTP() {
    ...
    try {
        ...
        const sendResult = await otp_service_1.otpService.sendOTP({ phoneNumber });
        ...
        const statusResult = await otp_service_1.otpService.checkStatus(phoneNumber);
        ...
        logger_1.logger.error('Test verify OTP failed', { error: error.message });
    }
}
async function testResendOTP() {
    ...
    try {
        ...
        const firstResult = await otp_service_1.otpService.sendOTP({ phoneNumber });
        ...
        const resendResult = await otp_service_1.otpService.resendOTP(phoneNumber);
        ...
        logger_1.logger.error('Test resend OTP failed', { error: error.message });
    }
}
async function testCheckStatus() {
    ...
    try {
        ...
        logger_1.logger.error('Test check status failed', { error: error.message });
    }
}
async function testInvalidPhoneNumber() {
    ...
    try {
        ...
        for (const invalidPhone of invalidNumbers) {
            ...
            const result = await otp_service_1.otpService.sendOTP({ phoneNumber: invalidPhone });
            if (!result.success) {
        ...
        console.error('\n❌ Error:', error.message);
        logger_1.logger.error('Test invalid phone number failed', { error: error.message });
    }
}
...
async function runAllTests() {
    ...
    try {
        ...
        console.error('Stack:', error.stack);
        logger_1.logger.error('Test suite failed', { error: error.message, stack: error.stack });
        process.exit(1);
    }
}
// CLI interface
