...
class AuthController {
    ...
    // ============================================================================
    async superAppWebviewLogin(req, res, next) {
        try {
            // รับ Token ชั่วคราวที่ Super App ส่งมาให้
            const { tempToken } = req.body;
            if (!tempToken) {
                throw new errors_1.BadRequestError('ກະລຸນາສົ່ງ Temp Token ຈາກ Super App');
            }
            // นำไปประมวลผลผ่าน Service
            const { customer, token } = await auth_service_1.default.processSuperAppLogin(tempToken);
